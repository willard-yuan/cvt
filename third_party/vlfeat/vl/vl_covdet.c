            ...
            int nin, const mxArray *in[])
{
  ...
  image = (float*) mxGetData(IN(I)) ; /* 将图像以列拼接成成一维数组 */
  ...
  switch (descriptorType) {
    ...
    case VL_COVDET_DESC_SIFT :
	  ...
	  /* 添加的测试代码 */
	  if (verbose) {
		  mexPrintf("descriptorType: %s\n", "VL_COVDET_DESC_SIFT") ;
		  mexPrintf("vl_covdet: VL_COVDET_DESC_SIFT: "
		  "patchResolution=%d, patchRelativeExtent=%g, patchRelativeSmoothing=%g\n",
   ...
   * -------------------------------------------------------------- */
  {
    ...
    if (verbose) {
      VL_PRINTF("vl_covdet: doubling image: %s, image size: numRows = %d, numCols = %d\n",
                VL_YESNO(vl_covdet_get_first_octave(covdet) < 0), numRows, numCols) ;
    }

    /* process the image: vl_covdet_put_image to start processing a new image. It
     causes the detector to compute the scale space representation of the image, 
	 but does not compute the features yet */
    ...
    if (numUserFrames > 0) {
	  ...
	  /* vl_covdet_detect runs the detector. At this point features are 
      ...
      if (boundaryMargin > 0) {
		/* vl_covdet_drop_features_outside to drop features 
    ...
    if (estimateAffineShape) {
	  ...
	  /* vl_covdet_extract_affine_shape to compute the 
    ...
    if (estimateOrientation) {
	  ...
	  /* vl_covdet_extract_orientations to compute the 
    ...
    if (nout >= 2) {
      switch (descriptorType) {
        ...
        case VL_COVDET_DESC_PATCH:
        {
          ...
          for (i = 0 ; i < (signed)numFeatures ; ++i) {
			/* vl_covdet_extract_patch_for_frame to extract a 
