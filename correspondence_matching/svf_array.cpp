...
int cvtARRAY2CVMat(cv::Mat &descs, std::vector<cv::KeyPoint> &kpts, float* siftsData){
    ...
    int numEles = siftsData[0];
    if(numEles % 135 != 0) {
        return 0;
    }
    
    int numSIFTs = numEles/135.0;
    descs = cv::Mat::zeros(cv::Size(128, numSIFTs), CV_32FC1);
    int count = 1;
    for(int i = 0; i < descs.rows; i++) {
        for(int j = 0; j < descs.cols; j++) {
            descs.at<float>(i,j) = siftsData[count];
            ++count;
        }
        cv::KeyPoint curKpt;
        curKpt.pt.x = siftsData[count];
        ++count;
        curKpt.pt.y = siftsData[count];
        ++count;
        curKpt.angle = siftsData[count];
        ++count;
        curKpt.size = siftsData[count];
        ++count;
        curKpt.response = siftsData[count];
        ++count;
        curKpt.class_id = siftsData[count];
        ++count;
        curKpt.octave = siftsData[count];
        ++count;
        kpts.push_back(curKpt);
    }
    return 1;
}
...
int cvtSIFT2Array(cv::Mat &descs, std::vector<cv::KeyPoint> &kpts, float* siftsData) {
    ...
    if(descs.rows != kpts.size()){
        return 0;
    }
    
    int count = 0;
    siftsData[0] = (float)descs.rows*135;
    ++count;
    
    for(int i = 0; i < descs.rows; i++) {
        for(int j = 0; j < descs.cols; j++) {
            siftsData[count++] = descs.at<float>(i,j);
        }
        siftsData[count++] = (float)kpts.at(i).pt.x;
        siftsData[count++] = (float)kpts.at(i).pt.y;
        siftsData[count++] = (float)kpts.at(i).angle;
        siftsData[count++] = (float)kpts.at(i).size;
        siftsData[count++] = (float)kpts.at(i).response;
        siftsData[count++] = (float)kpts.at(i).class_id;
        siftsData[count++] = (float)kpts.at(i).octave;
    }
    return 1;
}

// siftsData的第一个元素是SIFT特征总元素数目
std::vector<cv::DMatch> getInliers(float* siftsData1, float* siftsData2) {
    ...
    cvtARRAY2CVMat(descs1, kpts1, siftsData1);
    cvtARRAY2CVMat(descs2, kpts2, siftsData2);
    
    std::vector<std::vector<cv::DMatch>> matches;
    std::vector<cv::DMatch> goodMatches2;
    siftMatcher.knnMatch(descs1, descs2, matches, 2);
    for (size_t i = 0; i < matches.size(); i++){
        if (matches[i][0].distance < siftRatio*matches[i][1].distance){
            goodMatches2.push_back(matches[i][0]);
        }
    }
    return getInliers(kpts1, kpts2, goodMatches2);
}
