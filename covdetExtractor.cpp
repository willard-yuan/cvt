                                                   ...
                                                   std::vector<std::vector<float>> &desctor)
{
    ...
    // vlfeat covdet.c 200th lines
    VlCovDetMethod method = VL_COVDET_METHOD_HESSIAN;  // VL_COVDET_METHOD_DOG
    ...
    double patchRelativeExtent = -1;  // patchRelativeExtent must be a posiive real
    double patchRelativeSmoothing = -1;  // patchRelativeSmoothing must be a non-negative real
    ...
    if (1) {
        ...
        if (estimateOrientation) {
            ...
            if (verbose && numFeaturesAfter > numFeaturesBefore) {
                VL_PRINTF("vl_covdet: %d duplicate features were created due to ambiguous "
